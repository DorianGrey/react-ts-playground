...
export default function register() {
  if (process.env.NODE_ENV === "production" && "serviceWorker" in navigator) {
    ...
    // FIXME: Insufficient types; URL can be constructed from window.location.
  ...
  navigator.serviceWorker
    ...
    .then(registration => {
      registration.onupdatefound = () => {
        const installingWorker = registration.installing as ServiceWorker; // Is OK => value is present if picked up in `onupdatefound`.
...
function checkValidServiceWorker(swUrl: string) {
  ...
  fetch(swUrl)
    .then(response => {
      ...
      if (
        response.status === 404 ||
        (response.headers.get("content-type") as string).indexOf(
          "javascript"
        ) === -1
      ) {
