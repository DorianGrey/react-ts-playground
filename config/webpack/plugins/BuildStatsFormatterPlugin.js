...
function validateOptions(options) {
  ...
  // `ignorePattern` is optional, but if it is provided, it has to be a RegExp object.
  {
    if (!!options.ignorePattern && !isRegExp(options.ignorePattern)) {
      throw new Error(
        "'ignorePattern', if provided, has to be a `RegExp` object."
      );
    }
  }
}
